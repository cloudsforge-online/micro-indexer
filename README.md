...
totals for one address (`src/jobs.ts`).
...
> (`src/server.ts`). The header states why — "ownership of an address is a fact `wallet`
> holds, so the indexer must not be the place that guesses at it" (`src/server.ts`).
...
> prevent (`src/migrations.ts`). It is computed at read time, every time.
...
> walked, after the node has been made to prove it still serves that block (`src/tokenstate.ts`).
...
> not zero, not null** (`src/reads.ts`). An indexer that started following at the tip knows
...
A **partial** unique index over the non-orphaned rows (`src/migrations.ts`). Inserting a
...
ancestor in one call, and **clears `confirmed_at`** while doing it (`src/store.ts`). The
reason is stated at `src/store.ts`: a confirmation is a statement about depth on the
...
(`src/migrations.ts`, column). `isReorgAlarming` from `contracts-chain` decides,
and the decision is **recorded on the `reorgs` row rather than recomputed**, so the row still says
what the policy was on the day after the policy changes (`src/migrations.ts`). Only an
...
`src/reads.ts`. The comment above it is the argument: **an EVM transaction that reverted is
...
(`src/reads.ts`). `tokenBalances` is a **`REPEATABLE READ` transaction**, because coverage,
...
"which is the only kind of statement worth making about money" (`src/reads.ts`).
...
`checkpoints.tip_height`, which is only what a provider last claimed (`src/store.ts`). Counting
...
| --- | --- | --- |
| `GET …/transactions/:hash/confirmations` | stored canonical **head** | `src/reads.ts` |
| `GET …/token-balances` | stored canonical **head**, in one `REPEATABLE READ` snapshot | `src/reads.ts` |
| `GET …/addresses/:address/activity` | `checkpoint.tipHeight` — the **provider-claimed tip** | `src/reads.ts` |
| `GET …/transactions/:hash` | `checkpoint.tipHeight` — the **provider-claimed tip** | `src/reads.ts` |
| `GET /tokens/…` | stored canonical **head**, *and* the node must still serve that block hash | `src/tokenstate.ts` |

`src/reads.ts` scopes its own claim correctly ("Two reads that exist because a consumer was
...
(`src/tokenstate.ts`) — the same check `src/evm.ts` makes at the start of every follow tick.
...
(`src/reads.ts`). That answer is derived from the whole history a halt says cannot be
vouched for; this one depends on exactly one block, and the hash check has just proved the node
still serves it (`src/tokenstate.ts`).
...
unprefixed** (`src/server.ts`, mounted). `/v1/…` is the estate convention and the
...
internal caller would have to follow (`src/server.ts`).
...
| --- | --- | --- | --- |
| `GET` | `/livez` | **no auth** | **static, deliberately** — a liveness probe that consults a dependency restarts a healthy process every time the database blinks, turning a brief outage into a rolling restart of the whole estate (`src/server.ts`, reasoning) |
| `GET` | `/readyz` | **no auth** | 200/503. A **soft** probe failure leaves the report degraded but still ready, because taking a whole product out of rotation over a non-essential upstream is worse than serving without it (`src/server.ts`) |
...
| `GET` | `/blocks/:chain/:network/:height` | **anonymous** | one canonical block with its transactions (`src/server.ts`, auth) |
| `POST` | `/watch/:chain/:network/:address` | `indexer:write` or admin | registers an address to be watched; records who asked (`src/server.ts`, auth, attribution) |
| `POST` | `/backfills/:chain/:network` | `indexer:write` or admin | opens a historical backfill stream (`src/server.ts`, auth) |
...
| Status | Code | Means |
| --- | --- | --- |
| 200 | — | the contract answered. Every field is nullable, and a null means **the contract does not implement that function** — a fixed-supply token has no `owner()`, `cap()` or `paused()` (`src/tokenstate.ts`) |
| 404 | `token_not_found` | this service asked the chain and there is no contract answering `totalSupply()` at the block it has walked. A deployment above the head reads as this, and that is honest (`src/server.ts`) |
...
have to hardcode a depth to interpret this API (`src/reads.ts`).
...
(`src/reads.ts`). A **token's** amount is deliberately left unformatted, because its decimals is
a call to the contract and a fact a token registry owns; guessing eighteen is how a six-decimal
stablecoin gets displayed a million times too small (`src/reads.ts`).
...
| --- | --- | --- | --- |
| `outbox.relay` | `stream` | 1s | one claims; keying on the event id would let two relays deliver one batch twice (`src/jobs.ts`) |
...
| `indexer.backfill` | `<chain>:<network>` | 2s | its own key, separate from the follower's, because they write different checkpoint streams and **a backfill must never block the tip** (`src/jobs.ts`, reasoning) |
...
(`src/jobs.ts`). Each tick calls `ctx.heartbeat()` so a long catch-up does not outlive its
lease and hand the same heights to a second replica (`src/jobs.ts`).
...
indexes up to `INDEXER_FOLLOW_BATCH_BLOCKS` blocks (`src/jobs.ts`).

Provider health is written **whatever happened**, because the tick that failed is the one whose
provider state an operator wants to see (`src/jobs.ts`). A `NotImplementedError` from an
...
(`src/jobs.ts`). A dead-lettered recurring job is deliberately not re-armed
(`src/jobs.ts`).
...
Three storage decisions carry the design (`src/migrations.ts`):
...
| --- | --- | --- |
| `blocks_canonical_height_uniq` (partial, `where status <> 'orphaned'`) | two live blocks at one height | it makes a **wrong reorg implementation** fail loudly. The handler is the thing being checked, so the check cannot live in it (`src/migrations.ts`) |
| `${CHAIN_CK}` / `${NETWORK_CK}` on **every** chain table | a sixth chain, or a third network, minted by a typo | one constant applied everywhere, so a new chain is a deliberate edit rather than a value that slips in (`src/migrations.ts`) |
| composite FKs carrying `(chain, network)` — `transactions_block_fk`, `logs_tx_fk`, `address_activity_tx_fk` | a cross-network reference | the reference itself carries the network, so a mis-written join cannot produce one (`src/migrations.ts`) |
| `transactions_block_ck check ((block_hash is null) = (block_height is null))` | half-known block placement | a transaction is either in a block or pending; "in a block whose height we do not know" is not a state (`src/migrations.ts`) |
| `address_activity_token_ck check ((asset_kind = 'token') = (token_address is not null))` | a token movement with no contract, or a native one with a contract | the two columns are one fact and the constraint says so (`src/migrations.ts`) |
| `address_activity_uniq (chain, network, tx_hash, entry_key)` | a duplicated movement on replay | **this is what makes every write idempotent**, which is what makes a lost lease cost only RPC traffic (`src/migrations.ts`) |
| `checkpoints_range_ck` / `checkpoints_range_order_ck` | a half-specified or inverted backfill range | (`src/migrations.ts`) |
| `reorgs_depth_ck check (depth > 0)` | a zero-depth reorg | a reorg of depth 0 is not a reorg; recording one would inflate the alert (`src/migrations.ts`) |
| `checkpoints.height` **nullable**, no zero sentinel | — | null means "nothing indexed on this stream yet". A zero would be indistinguishable from having indexed the genesis block, which on a chain whose genesis carries an allocation is a real and different statement (`src/migrations.ts`) |
| `provider_health` stores **only the host**, never the URL | — | an RPC provider's API key lives in the query string, and a table an operator reads over the shoulder of a screen share is not where it belongs (`src/migrations.ts`) |
...
balance-probing this service replaces (`src/env.ts`):

* a chain named in `INDEXER_CHAINS` **must** have an RPC variable (`src/env.ts`);
...
| --- | --- | --- |
| `PORT` | `4008` | must be a TCP port (`src/env.ts`) |
...
| `INDEXER_DATABASE_URL` | — | **required** (`src/env.ts`). Rule 1 — CI greps for a second connection string |
| `INDEXER_DATABASE_POOL_MAX` | `10` | integer 1–200 (`src/env.ts`) |
| `IDENTITY_JWKS_URL` | — | **required**. Unreachable at runtime → every domain route answers 503, not 401 (`src/env.ts`) |
| `IDENTITY_ISSUER` | — | **required**. Wrong → the whole surface is 401 (`src/env.ts`) |
| `OUTBOX_SIGNING_SECRET` | — | **required, ≥24 chars, placeholders refused.** Shipped **empty** in `.env.example`, so a copied file refuses to boot until it is filled — deliberate (`src/env.ts`) |
| `INSTANCE_ID` | hostname | names this replica in `jobs.locked_by` (`src/env.ts`) |
| `INDEXER_CHAINS` | `` (none) | `<chain>:<network>` pairs. **Empty is legal**: a replica may serve reads and follow nothing (`src/env.ts`). A duplicate entry is a boot failure |
| `INDEXER_RPC_<CHAIN>_<NETWORK>` | — | **required for each configured chain.** `name=url` pairs in preference order; the pool reorders by health and declaration order breaks ties. **The name is the primary key of `provider_health` and the `provider` metric label**, so two endpoints at one host must be named by hand rather than collapsing into one health row that averages a working provider with a broken one (`src/env.ts`, reasoning). Only the **host** is ever stored or logged |
| `INDEXER_START_HEIGHT_<CHAIN>_<NETWORK>` | tip − 2× the confirmation depth | where a cold start begins. The default is the smallest window that can still watch a deposit through its whole confirmation life; **older history is a backfill job, not a cold start**, because a cold start that walks five million blocks before it serves anything is a service that is never ready (`src/env.ts`, reasoning) |
...
provider API key lives (`src/env.ts`).
...
| `micro-identity` | its JWKS at `IDENTITY_JWKS_URL` (`src/index.ts`) | **soft readiness probe**, so this replica stays in rotation. Domain routes answer 503 rather than 401 while the verifier is unreachable |
...
table out of `src/server.ts` and fails if any path it requests is not one this service
...
(`src/server.ts`). `micro-wallet` composes this service with `ledger` and `custody`.
...
(`src/hearth.test.ts`) — but a green run without a node has **not** exercised the real chain, and
...
* **Two documents over-generalise the head-versus-tip rule, and both are wrong as written.**
  * `docs/ecosystem/18-build-status.md` says confirmations count against the stored canonical
    ...
    `checkpoint.tipHeight` (`src/reads.ts`).
  * `src/migrations.ts` says the reverse — that confirmations are derived "from
    `checkpoints.tip_height`" — which was true before §3.3j added the two head-based reads and is
    now half wrong.

  `src/reads.ts` is the accurate statement. **Both are reported, not edited**: this task's
...
* **A token observation is up to nine RPC calls and nothing caches it.** One
  ...
  field (`src/tokenstate.ts`). `cache-control: no-store` is estate-wide and right — a
...
* **`/tokens/…` is the one read that needs a provider**, so a replica configured with no
  ...
  other read from the database (`src/index.ts`, `src/tokenstate.ts`). That is deliberate
...
* **`mintAuthority` over-reports rather than under-reports.** A contract with an `owner()` and no
  ...
  costs them money (`src/tokenstate.ts`).
...
* **Not every chain family is implemented.** `NotImplementedError` carries a `family` and a `phase`
  (`src/jobs.ts`); a deployment that names an unimplemented chain in `INDEXER_CHAINS` gets a
  dead-lettered follow job rather than a boot failure. The dead row is the record, and
  `jobs_overdue` is what surfaces it.
* **`/metrics` is unauthenticated** (`src/server.ts`), unlike `micro-beacon`'s. Anything that
...
* **`confirmed` on an activity row is depth-only.** Unlike `/confirmations`, it does not require
  `status = 'success'` and does not consult `halted` (`src/reads.ts` versus). A
...
* **The unversioned spelling is permanent.** Both `/v1/…` and bare paths are served
  (`src/server.ts`); the estate has no gateway route map, so nothing decides which is public
